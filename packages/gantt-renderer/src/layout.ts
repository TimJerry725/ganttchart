...
import type { TimeScale, RowLayout, BarLayout, LinkLayout, Viewport } from './types';
...
export class LayoutEngine {
  ...
  calculateLayout(
    tasks: Task[],
    links: Link[],
    scheduledTasks: Map<TaskId, { scheduledStart: Date; scheduledEnd: Date }>,
    timeScale: TimeScale,
    viewport: Viewport,
    ...
    const calculateRows = (taskList: Task[], level: number): void => {
      for (const task of taskList) {
        ...
        if (scheduled) {
          const barX = this.dateToX(scheduled.scheduledStart, timeScale);
          const barWidth = this.dateToX(scheduled.scheduledEnd, timeScale) - barX;
          ...
          bars.set(task.id, {
            ...
            progressX: task.progress
              ? barX + (barWidth * task.progress) / 100
              : undefined,
